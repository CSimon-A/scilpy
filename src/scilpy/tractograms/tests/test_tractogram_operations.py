...
def test_robust_operations():
    ...
    assert len(out_diff) == 1, f"Difference expected 1 line, got {len(out_diff)}"
    assert np.allclose(out_diff[0], line_x), "Difference did not return the X line"
    ...
    assert len(out_int) == 1, f"Intersection expected 1 line, got {len(out_int)}"
    assert np.allclose(out_int[0], line_y), "Intersection did not return the Y line"
    ...
    assert len(out_int_rev) == 1, "Intersection failed to match reversed streamlines."
    ...
    assert len(out_diff_rev) == 0, "Difference failed to remove reversed streamline."
